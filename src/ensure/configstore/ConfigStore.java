...
public class ConfigStore {
  ...
  public static int Get () {
    return configStore.parseFile("./data/base.vze");
  }

  private int parseFile (String argv) {
    int n = 40;
    ...
    try {
      ...
      fr = new FileReader(argv);
      br = new BufferedReader(fr);

      String sCurrentLine;

      br = new BufferedReader(new FileReader(argv));
        ...
        if (sCurrentLine.equals("[Cage]")) {
          while (!(sCurrentLine = br.readLine()).equals("[END OF Cage]")) {
            ...
            if (Integer.parseInt(tokens[0]) < n && Integer.parseInt(tokens[1]) < n) {
              switch (tokens[3]) {
                case "WaterHabitat":
                  Zoo.Get(n).setCell(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[0]), new WaterHabitat(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[0]), Integer.parseInt(tokens[2])));
                  break;
                case "LandHabitat":
                  Zoo.Get(n).setCell(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[0]), new LandHabitat(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[0]), Integer.parseInt(tokens[2])));
                  break;
                case "FlyingHabitat":
            ...
            switch (tokens[2]) {
              ...
              default:
                if (tokens[3].equals("RoadExit")) {
                  Zoo.Get(n).setCell(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[0]), new Road(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[0]), "RoadExit", '_'));
                } else {
                  Zoo.Get(n).setCell(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[0]), new Road(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[0]), "RoadEntrance", '_'));
                }
                break;
            }
          }
